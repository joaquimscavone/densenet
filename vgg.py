...
def create(epochs=250, architecture=19, batch_size=1, MLPinput=4096, MLPhidden=4096, discart_prop=0, optimizer='sgd'):
	...
	checkpoint = ModelCheckpoint('t1_best_weights.hdf5', monitor='val_acc', verbose=1, save_best_only=True,save_weights_only=True, mode='max')
	...
	model.save_weights('t1_end_weights.hdf5', True)
	file_train_history = open('t1_history.json', 'w')
	...
	print("Descongelando últimas camadas das convoluções!")
	
	

	checkpoint = ModelCheckpoint('t2_best_weights.hdf5', monitor='val_acc', verbose=1, save_best_only=True,save_weights_only=True, mode='max')
	history=model.fit(X_train, y_train,
	          batch_size=batch_size,
	          epochs=epochs,
	          callbacks=[checkpoint,],
	          verbose=1,
	          validation_data=(X_valid, y_valid))
	model.save_weights('t2_end_weights.hdf5', True)
	file_train_history = open('t2_history.json', 'w')
	file_train_history.write(json.dumps(history.history))
	file_train_history.close()
	score = model.evaluate(X_test, y_test, verbose=0)
	print('Test loss:', score[0])
